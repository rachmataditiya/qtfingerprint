...
public:
    ...
    /**
     * @brief Capture fingerprint template from device (single capture)
    ...
    /**
     * @brief Enroll fingerprint (full enrollment with multiple scans)
     * This uses fp_device_enroll_sync which performs all required scans internally
     * @param[out] templateData Enrolled template data (FP3 format)
     * @return true on success
     */
    bool enrollFingerprint(std::vector<uint8_t>& templateData);
    
    /**
     * @brief Match captured fingerprint with stored template (local matching using libfprint)
     * This will capture current fingerprint and match with stored template in one step
     ...
     * @return true on success
     */
    bool identifyUser(const std::vector<std::pair<int, std::vector<uint8_t>>>& templates, int& matchedIndex, int& score);
